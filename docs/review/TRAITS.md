# Traits

*118 traits. The player rolls one inborn **physical**, one **personality** and one **behavioral** at creation; **acquired** ones are earned during a run; **racial** ones come with the birth.*

*`pressure` shifts the emotional baseline that decay pulls toward: negative is toward that element's klesha, positive toward its wisdom. `bond` tags are what RelationshipSystem scores party rapport on.*

*Edit names and descriptions between the anchors. The mechanical line is generated — change it in `traits.json`.*

---


# Physical  (12)


## attractive

`stats: charm+1  ·  pressure: earth-5  ·  bond: sociable  ·  on 3 companions`

**Name**

<!--@ traits.json | attractive | name -->
Attractive
<!--@end-->


**Description**

<!--@ traits.json | attractive | description -->
A pleasing appearance opens many doors.
<!--@end-->


## chronic_pain

`stats: constitution-1  ·  skills: yoga+1  ·  pressure: water-10, earth-5  ·  bond: hardship  ·  purged by ritual 5  ·  on 0 companions`

**Name**

<!--@ traits.json | chronic_pain | name -->
Chronic Pain
<!--@end-->


**Description**

<!--@ traits.json | chronic_pain | description -->
Dull, throbbing, always there.
<!--@end-->


## clubfooted

`stats: finesse-1  ·  bond: hardship  ·  on 0 companions`

**Name**

<!--@ traits.json | clubfooted | name -->
Clubfooted
<!--@end-->


**Description**

<!--@ traits.json | clubfooted | description -->
An awkward gait that hampers quick movement.
<!--@end-->


## frail

`stats: constitution-1  ·  bond: hardship  ·  on 1 companions`

**Name**

<!--@ traits.json | frail | name -->
Frail
<!--@end-->


**Description**

<!--@ traits.json | frail | description -->
A fragile constitution, easily tired and slow to heal.
<!--@end-->


## hard_of_hearing

`stats: awareness-1  ·  bond: hardship  ·  on 1 companions`

**Name**

<!--@ traits.json | hard_of_hearing | name -->
Hard of Hearing
<!--@end-->


**Description**

<!--@ traits.json | hard_of_hearing | description -->
Weary of asking others to repeat themselves, they begrudgingly accepted their condition.
<!--@end-->


## impaired_vision

`stats: awareness-1  ·  bond: hardship  ·  opposed: sharp_eyed  ·  on 0 companions`

**Name**

<!--@ traits.json | impaired_vision | name -->
Impaired Vision
<!--@end-->


**Description**

<!--@ traits.json | impaired_vision | description -->
Struggles in the dark, misses details and enemies.
<!--@end-->


## iron_stomach

`bond: sociable  ·  on 3 companions`

**Name**

<!--@ traits.json | iron_stomach | name -->
Iron Stomach
<!--@end-->


**Description**

<!--@ traits.json | iron_stomach | description -->
Five second rule!
<!--@end-->


## light_sleeper

`stats: awareness+1  ·  pressure: air-5  ·  on 0 companions`

**Name**

<!--@ traits.json | light_sleeper | name -->
Light Sleeper
<!--@end-->


**Description**

<!--@ traits.json | light_sleeper | description -->
Sleep with one eye open, gripping the pillow tight.
<!--@end-->


## night_owl

`opposed: early_riser  ·  on 3 companions`

**Name**

<!--@ traits.json | night_owl | name -->
Night Owl
<!--@end-->


**Description**

<!--@ traits.json | night_owl | description -->
The owl of wisdom flies after midnight.
<!--@end-->


## quick

`stats: finesse+1  ·  on 6 companions`

**Name**

<!--@ traits.json | quick | name -->
Quick
<!--@end-->


**Description**

<!--@ traits.json | quick | description -->
Naturally fast reflexes and light feet.
<!--@end-->


## sharp_eyed

`stats: awareness+1  ·  pressure: air+5  ·  bond: scholarly  ·  opposed: impaired_vision  ·  on 1 companions`

**Name**

<!--@ traits.json | sharp_eyed | name -->
Sharp-eyed
<!--@end-->


**Description**

<!--@ traits.json | sharp_eyed | description -->
Nothing escapes their notice.
<!--@end-->


## strong

`stats: strength+1  ·  bond: martial  ·  on 1 companions`

**Name**

<!--@ traits.json | strong | name -->
Strong
<!--@end-->


**Description**

<!--@ traits.json | strong | description -->
Doors seem a little narrow.
<!--@end-->


---


# Personality  (21)


## brave

`pressure: air+10, water+5  ·  bond: martial  ·  opposed: timid  ·  on 10 companions`

**Name**

<!--@ traits.json | brave | name -->
Brave
<!--@end-->


**Description**

<!--@ traits.json | brave | description -->
Faces danger without flinching.
<!--@end-->


## clear_eyed

`pressure: water+10, space+5  ·  bond: scholarly  ·  opposed: superstitious  ·  on 1 companions`

**Name**

<!--@ traits.json | clear_eyed | name -->
Clear-eyed
<!--@end-->


**Description**

<!--@ traits.json | clear_eyed | description -->
Sees what is in front of them without dressing it up.
<!--@end-->


## content

`pressure: earth+10, fire+5  ·  bond: order  ·  opposed: covetous, braggart, vain  ·  on 0 companions`

**Name**

<!--@ traits.json | content | name -->
Content
<!--@end-->


**Description**

<!--@ traits.json | content | description -->
When you don't have what you like, like what you have.
<!--@end-->


## covetous

`pressure: air-10, earth-5  ·  opposed: content, celebrant  ·  purged by yoga 3  ·  on 0 companions`

**Name**

<!--@ traits.json | covetous | name -->
Covetous
<!--@end-->


**Description**

<!--@ traits.json | covetous | description -->
Measures their life against other people's, and comes up short every time.
<!--@end-->


## craven

`stats: finesse+1  ·  skills: might-1  ·  pressure: fire-5, earth-5  ·  opposed: brave  ·  purged by yoga 14  ·  on 0 companions`

**Name**

<!--@ traits.json | craven | name -->
Craven
<!--@end-->


**Description**

<!--@ traits.json | craven | description -->
Runs first and reasons afterwards. Whatever else is true, nothing has ever caught them.
<!--@end-->


## curious

`skills: learning+1  ·  pressure: space+10  ·  bond: scholarly, wonder  ·  opposed: incurious  ·  on 15 companions`

**Name**

<!--@ traits.json | curious | name -->
Curious
<!--@end-->


**Description**

<!--@ traits.json | curious | description -->
Irresistibly drawn to the unknown.
<!--@end-->


## forgetful

`skills: learning-1  ·  pressure: space-5  ·  opposed: sharp_memory  ·  on 1 companions`

**Name**

<!--@ traits.json | forgetful | name -->
Forgetful
<!--@end-->


**Description**

<!--@ traits.json | forgetful | description -->
Details slip away; rarely remembers what was said.
<!--@end-->


## generous

`stats: charm+1  ·  pressure: earth+10  ·  bond: sociable  ·  opposed: scrimper, greedy  ·  on 8 companions`

**Name**

<!--@ traits.json | generous | name -->
Generous
<!--@end-->


**Description**

<!--@ traits.json | generous | description -->
Gives freely, expects little in return.
<!--@end-->


## greedy

`skills: trade+1  ·  pressure: fire-10, earth-5  ·  bond: vice  ·  opposed: generous  ·  purged by yoga, ritual 3  ·  on 9 companions`

**Name**

<!--@ traits.json | greedy | name -->
Greedy
<!--@end-->


**Description**

<!--@ traits.json | greedy | description -->
Always calculating what they stand to gain.
<!--@end-->


## grudge_bearer

`pressure: water-10, air-5  ·  bond: solitary  ·  purged by yoga, ritual 4  ·  on 1 companions`

**Name**

<!--@ traits.json | grudge_bearer | name -->
Grudge-bearer
<!--@end-->


**Description**

<!--@ traits.json | grudge_bearer | description -->
Forgets nothing that was done to them, and keeps the ledger current.
<!--@end-->


## hot_tempered

`stats: charm-1  ·  pressure: water-15  ·  opposed: patient  ·  purged by yoga, ritual 3  ·  on 6 companions`

**Name**

<!--@ traits.json | hot_tempered | name -->
Hot-tempered
<!--@end-->


**Description**

<!--@ traits.json | hot_tempered | description -->
Quick to anger, slow to forgive.
<!--@end-->


## incurious

`pressure: space-10  ·  bond: solitary  ·  opposed: curious  ·  purged by yoga 3  ·  on 0 companions`

**Name**

<!--@ traits.json | incurious | name -->
Incurious
<!--@end-->


**Description**

<!--@ traits.json | incurious | description -->
At first, asking questions was difficult, then it became impossible.
<!--@end-->


## melancholic

`pressure: water-10  ·  bond: grief, solitary  ·  purged by yoga, ritual 2  ·  on 3 companions`

**Name**

<!--@ traits.json | melancholic | name -->
Melancholic
<!--@end-->


**Description**

<!--@ traits.json | melancholic | description -->
A deep sadness runs beneath the surface.
<!--@end-->


## patient

`pressure: water+10, fire+5  ·  bond: order  ·  opposed: hot_tempered, duelist  ·  on 15 companions`

**Name**

<!--@ traits.json | patient | name -->
Patient
<!--@end-->


**Description**

<!--@ traits.json | patient | description -->
Once you endure the first impulse, you can wait almost indefinitely.
<!--@end-->


## sharp_memory

`skills: learning+1  ·  pressure: space+5  ·  bond: scholarly  ·  opposed: forgetful  ·  on 7 companions`

**Name**

<!--@ traits.json | sharp_memory | name -->
Sharp Memory
<!--@end-->


**Description**

<!--@ traits.json | sharp_memory | description -->
Remembers everything — names, faces, slights.
<!--@end-->


## stubborn

`pressure: earth-10  ·  bond: order  ·  purged by yoga 4  ·  on 19 companions`

**Name**

<!--@ traits.json | stubborn | name -->
Stubborn
<!--@end-->


**Description**

<!--@ traits.json | stubborn | description -->
Hard to sway in any direction — for good or ill.
<!--@end-->


## suspicious

`stats: charm-1, awareness+1  ·  pressure: air-10  ·  bond: solitary  ·  opposed: trusting  ·  purged by yoga 3  ·  on 4 companions`

**Name**

<!--@ traits.json | suspicious | name -->
Suspicious
<!--@end-->


**Description**

<!--@ traits.json | suspicious | description -->
Trusts no one at face value.
<!--@end-->


## timid

`stats: charm-1  ·  pressure: air-15  ·  bond: solitary  ·  opposed: brave  ·  purged by yoga, ritual 3  ·  on 0 companions`

**Name**

<!--@ traits.json | timid | name -->
Timid
<!--@end-->


**Description**

<!--@ traits.json | timid | description -->
Easily frightened; prefers to avoid confrontation.
<!--@end-->


## trusting

`stats: charm+1, awareness-1  ·  pressure: air+10  ·  bond: sociable  ·  opposed: suspicious, paranoid  ·  on 3 companions`

**Name**

<!--@ traits.json | trusting | name -->
Trusting
<!--@end-->


**Description**

<!--@ traits.json | trusting | description -->
Extends goodwill freely, perhaps too freely.
<!--@end-->


## vain

`stats: charm+1  ·  pressure: earth-10  ·  bond: sociable  ·  opposed: content  ·  purged by yoga, ritual 3  ·  on 7 companions`

**Name**

<!--@ traits.json | vain | name -->
Vain
<!--@end-->


**Description**

<!--@ traits.json | vain | description -->
Takes great pride in their appearance and reputation.
<!--@end-->


## warm_hearted

`stats: charm+1  ·  pressure: fire+10, water+5  ·  bond: sociable  ·  on 2 companions`

**Name**

<!--@ traits.json | warm_hearted | name -->
Warm-hearted
<!--@end-->


**Description**

<!--@ traits.json | warm_hearted | description -->
Easy to like, and easy about liking other people.
<!--@end-->


---


# Behavioral  (31)


## ascetic

`bond: devotion, order  ·  opposed: gourmand, night_drinker, flirt, drunk, bhang_enjoyer  ·  on 3 companions`

**Name**

<!--@ traits.json | ascetic | name -->
Ascetic
<!--@end-->


**Description**

<!--@ traits.json | ascetic | description -->
Eats little, sleeps hard, and wants less than the people around them.
<!--@end-->


## beast_tender

`bond: beasts  ·  on 0 companions`

**Name**

<!--@ traits.json | beast_tender | name -->
Beast-tender
<!--@end-->


**Description**

<!--@ traits.json | beast_tender | description -->
Good with animals, and they appear to know it right away.
<!--@end-->


## bhang_enjoyer

`pressure: space-5, water+5  ·  bond: vice, wonder  ·  opposed: ascetic  ·  on 0 companions`

**Name**

<!--@ traits.json | bhang_enjoyer | name -->
Bhang Enjoyer
<!--@end-->


**Description**

<!--@ traits.json | bhang_enjoyer | description -->
Takes it only at the festivals - and isn't every day a festival of its own kind?
<!--@end-->


## bird_lover

`bond: beasts  ·  on 0 companions`

**Name**

<!--@ traits.json | bird_lover | name -->
Bird Lover
<!--@end-->


**Description**

<!--@ traits.json | bird_lover | description -->
Aren't those beady eyes just adorable?
<!--@end-->


## braggart

`bond: sociable, martial  ·  opposed: content  ·  on 1 companions`

**Name**

<!--@ traits.json | braggart | name -->
Braggart
<!--@end-->


**Description**

<!--@ traits.json | braggart | description -->
Improves the story a little every time it is told.
<!--@end-->


## celebrant

`pressure: air+10  ·  bond: arts, sociable  ·  opposed: covetous  ·  on 0 companions`

**Name**

<!--@ traits.json | celebrant | name -->
Celebrant
<!--@end-->


**Description**

<!--@ traits.json | celebrant | description -->
Takes uncomplicated pleasure in other people's luck.
<!--@end-->


## collector

`pressure: fire-5  ·  bond: order  ·  on 0 companions`

**Name**

<!--@ traits.json | collector | name -->
Collector
<!--@end-->


**Description**

<!--@ traits.json | collector | description -->
Compelled to gather and catalogue curious objects.
<!--@end-->


## debtor

`bond: vice  ·  on 0 companions`

**Name**

<!--@ traits.json | debtor | name -->
Debtor
<!--@end-->


**Description**

<!--@ traits.json | debtor | description -->
Owes money somewhere, to someone with an unusually long memory.
<!--@end-->


## dreamer

`stats: focus+1  ·  pressure: space-5  ·  bond: wonder, arts  ·  on 2 companions`

**Name**

<!--@ traits.json | dreamer | name -->
Dreamer
<!--@end-->


**Description**

<!--@ traits.json | dreamer | description -->
Feels more at home among the visions than the flesh.
<!--@end-->


## duelist

`bond: martial  ·  opposed: patient  ·  on 1 companions`

**Name**

<!--@ traits.json | duelist | name -->
Duelist
<!--@end-->


**Description**

<!--@ traits.json | duelist | description -->
Prides themselves on never turning down a challenge.
<!--@end-->


## early_riser

`bond: order  ·  opposed: night_owl  ·  on 1 companions`

**Name**

<!--@ traits.json | early_riser | name -->
Early Riser
<!--@end-->


**Description**

<!--@ traits.json | early_riser | description -->
Early bird gets to gloat when others wake up.
<!--@end-->


## fastidious

`pressure: earth-5  ·  bond: order  ·  on 6 companions`

**Name**

<!--@ traits.json | fastidious | name -->
Fastidious
<!--@end-->


**Description**

<!--@ traits.json | fastidious | description -->
Particular about cleanliness and order; unsettled by squalor.
<!--@end-->


## flirt

`bond: sociable  ·  opposed: ascetic  ·  on 3 companions`

**Name**

<!--@ traits.json | flirt | name -->
Flirt
<!--@end-->


**Description**

<!--@ traits.json | flirt | description -->
Incurably charming, and worst of all at the least convenient moments.
<!--@end-->


## gambler

`stats: luck+1  ·  pressure: fire-10  ·  bond: vice  ·  purged by yoga 3  ·  on 3 companions`

**Name**

<!--@ traits.json | gambler | name -->
Gambler
<!--@end-->


**Description**

<!--@ traits.json | gambler | description -->
Did you know most gamblers walk away from the table right before a big win?
<!--@end-->


## gossip

`bond: sociable  ·  opposed: secret_bearer  ·  on 2 companions`

**Name**

<!--@ traits.json | gossip | name -->
Gossip
<!--@end-->


**Description**

<!--@ traits.json | gossip | description -->
Knows everyone's business within a day of arriving anywhere.
<!--@end-->


## gourmand

`pressure: fire-5  ·  bond: vice, sociable  ·  opposed: ascetic  ·  on 1 companions`

**Name**

<!--@ traits.json | gourmand | name -->
Gourmand
<!--@end-->


**Description**

<!--@ traits.json | gourmand | description -->
Takes great pleasure in fine food and good company at table.
<!--@end-->


## homesick

`bond: solitary, grief  ·  on 2 companions`

**Name**

<!--@ traits.json | homesick | name -->
Homesick
<!--@end-->


**Description**

<!--@ traits.json | homesick | description -->
The farther away you are from your home, the easier it is to see it through rose-colored glasses.
<!--@end-->


## hunter

`skills: ranged+1  ·  bond: beasts, solitary  ·  opposed: sworn_vegetarian, pet_lover  ·  on 2 companions`

**Name**

<!--@ traits.json | hunter | name -->
Hunter
<!--@end-->


**Description**

<!--@ traits.json | hunter | description -->
Reads ground the way other people read letters. Knows what passed, how long ago, and whether it was hungry.
<!--@end-->


## mourner

`bond: grief, devotion  ·  on 2 companions`

**Name**

<!--@ traits.json | mourner | name -->
Mourner
<!--@end-->


**Description**

<!--@ traits.json | mourner | description -->
Keeps the dead's days. Leaves food out on the right nights, wherever they are.
<!--@end-->


## night_drinker

`pressure: fire-10  ·  bond: vice  ·  opposed: ascetic  ·  purged by yoga 3  ·  on 1 companions`

**Name**

<!--@ traits.json | night_drinker | name -->
Night Drinker
<!--@end-->


**Description**

<!--@ traits.json | night_drinker | description -->
When the sun goes down, the glass goes up.
<!--@end-->


## paranoid

`stats: awareness+1, charm-1  ·  pressure: air-15  ·  bond: solitary  ·  opposed: trusting  ·  purged by yoga, ritual 4  ·  on 1 companions`

**Name**

<!--@ traits.json | paranoid | name -->
Paranoid
<!--@end-->


**Description**

<!--@ traits.json | paranoid | description -->
Always watching. Always expecting the worst. Wait, did you hear that?
<!--@end-->


## pet_lover

`pressure: water+5  ·  bond: beasts, sociable  ·  opposed: hunter  ·  on 0 companions`

**Name**

<!--@ traits.json | pet_lover | name -->
Pet-lover
<!--@end-->


**Description**

<!--@ traits.json | pet_lover | description -->
Has never once passed an animal without stopping.
<!--@end-->


## pilgrim

`bond: devotion, wonder  ·  on 2 companions`

**Name**

<!--@ traits.json | pilgrim | name -->
Pilgrim
<!--@end-->


**Description**

<!--@ traits.json | pilgrim | description -->
The party schedule will wait, the shrine must be circumambulated.
<!--@end-->


## poet

`bond: arts, wonder  ·  on 0 companions`

**Name**

<!--@ traits.json | poet | name -->
Poet
<!--@end-->


**Description**

<!--@ traits.json | poet | description -->
Composes constantly in their head, and occasionally, without warning, out loud.
<!--@end-->


## present

`stats: awareness+1  ·  pressure: space+10  ·  bond: wonder, scholarly  ·  on 0 companions`

**Name**

<!--@ traits.json | present | name -->
Present
<!--@end-->


**Description**

<!--@ traits.json | present | description -->
Here and now.
<!--@end-->


## scrimper

`bond: order  ·  opposed: generous  ·  on 1 companions`

**Name**

<!--@ traits.json | scrimper | name -->
Scrimper
<!--@end-->


**Description**

<!--@ traits.json | scrimper | description -->
Cannot part with money without a grimace.
<!--@end-->


## secret_bearer

`bond: solitary  ·  opposed: gossip  ·  on 3 companions`

**Name**

<!--@ traits.json | secret_bearer | name -->
Secret-bearer
<!--@end-->


**Description**

<!--@ traits.json | secret_bearer | description -->
Carrying something they have told nobody, and have no plans to.
<!--@end-->


## storyteller

`skills: persuasion+1  ·  pressure: air+5  ·  bond: arts, sociable  ·  on 0 companions`

**Name**

<!--@ traits.json | storyteller | name -->
Storyteller
<!--@end-->


**Description**

<!--@ traits.json | storyteller | description -->
Always has a tale ready — some true, most embellished.
<!--@end-->


## superstitious

`pressure: space-10  ·  bond: wonder  ·  opposed: clear_eyed  ·  purged by yoga, ritual 2  ·  on 2 companions`

**Name**

<!--@ traits.json | superstitious | name -->
Superstitious
<!--@end-->


**Description**

<!--@ traits.json | superstitious | description -->
There are no coincidences, only signs!
<!--@end-->


## sworn_vegetarian

`bond: devotion, order  ·  opposed: hunter  ·  on 0 companions`

**Name**

<!--@ traits.json | sworn_vegetarian | name -->
Sworn Vegetarian
<!--@end-->


**Description**

<!--@ traits.json | sworn_vegetarian | description -->
Will not eat anything that had a mother.
<!--@end-->


## tea_ritualist

`pressure: water+10  ·  bond: sociable, order  ·  on 1 companions`

**Name**

<!--@ traits.json | tea_ritualist | name -->
Tea Ritualist
<!--@end-->


**Description**

<!--@ traits.json | tea_ritualist | description -->
The drinking is only the last part of the pleasure.
<!--@end-->


---


# Acquired  (26)


## addiction

`pressure: fire-15, earth-5  ·  bond: vice, hardship  ·  purged by yoga, ritual 4  ·  on 3 companions`

**Name**

<!--@ traits.json | addiction | name -->
Addiction
<!--@end-->


**Description**

<!--@ traits.json | addiction | description -->
Dependent on something — herbs, drink, or something stranger.
<!--@end-->


## ash_marked

`skills: ritual+1  ·  pressure: earth+5  ·  bond: devotion  ·  on 0 companions`

**Name**

<!--@ traits.json | ash_marked | name -->
Ash-marked
<!--@end-->


**Description**

<!--@ traits.json | ash_marked | description -->
Has done the purification rites often enough to stop finding them strange.
<!--@end-->


## beauty_struck

`pressure: space+10, fire+5  ·  bond: wonder, arts  ·  on 0 companions`

**Name**

<!--@ traits.json | beauty_struck | name -->
Beauty-struck
<!--@end-->


**Description**

<!--@ traits.json | beauty_struck | description -->
Saw something that quietly reordered what they had thought possible.
<!--@end-->


## blood_handed

`pressure: water-10, earth-5  ·  bond: martial, hardship  ·  opposed: merciful  ·  purged by ritual 5  ·  on 1 companions`

**Name**

<!--@ traits.json | blood_handed | name -->
Blood-handed
<!--@end-->


**Description**

<!--@ traits.json | blood_handed | description -->
Has taken lives that weigh on the soul.
<!--@end-->


## bloodied

`pressure: water-5  ·  bond: martial  ·  on 0 companions`

**Name**

<!--@ traits.json | bloodied | name -->
Slayer
<!--@end-->


**Description**

<!--@ traits.json | bloodied | description -->
Has killed something that everyone agreed could not be killed.
<!--@end-->


## composed

`pressure: fire+5, water+5  ·  bond: order  ·  on 7 companions`

**Name**

<!--@ traits.json | composed | name -->
Composed
<!--@end-->


**Description**

<!--@ traits.json | composed | description -->
A hard-won calm. Seldom ruffled by what once would have stung.
<!--@end-->


## death_touched

`pressure: water+5, space-5  ·  bond: hardship, wonder  ·  on 0 companions`

**Name**

<!--@ traits.json | death_touched | name -->
Death-touched
<!--@end-->


**Description**

<!--@ traits.json | death_touched | description -->
Was owed to something, and was not collected. It has changed how they walk into rooms.
<!--@end-->


## devout

`skills: yoga+1, ritual+1  ·  pressure: space+10  ·  bond: devotion  ·  opposed: lapsed  ·  on 6 companions`

**Name**

<!--@ traits.json | devout | name -->
Devout
<!--@end-->


**Description**

<!--@ traits.json | devout | description -->
A sincere and practiced faith shapes every act.
<!--@end-->


## drunk

`pressure: fire-10, earth-5  ·  bond: vice, hardship  ·  opposed: ascetic  ·  purged by yoga, ritual 4  ·  on 0 companions`

**Name**

<!--@ traits.json | drunk | name -->
Drunk
<!--@end-->


**Description**

<!--@ traits.json | drunk | description -->
Past enjoying it, now it's just an everyday fact.
<!--@end-->


## enlightened_insight

`stats: focus+1  ·  pressure: space+15, fire+5  ·  bond: wonder, devotion  ·  on 0 companions`

**Name**

<!--@ traits.json | enlightened_insight | name -->
Enlightened Insight
<!--@end-->


**Description**

<!--@ traits.json | enlightened_insight | description -->
A rare flash of clarity left a permanent mark.
<!--@end-->


## grief_struck

`pressure: water-20  ·  bond: grief  ·  purged by yoga, ritual 3  ·  on 1 companions`

**Name**

<!--@ traits.json | grief_struck | name -->
Grief-struck
<!--@end-->


**Description**

<!--@ traits.json | grief_struck | description -->
Carrying a loss that has not yet found its shape.
<!--@end-->


## harrowed

`pressure: water-10, air-10  ·  bond: hardship, solitary  ·  purged by yoga, ritual 4  ·  on 0 companions`

**Name**

<!--@ traits.json | harrowed | name -->
Harrowed
<!--@end-->


**Description**

<!--@ traits.json | harrowed | description -->
Something happened. They have not described it, and are not going to.
<!--@end-->


## haunted

`pressure: water-15, space-5  ·  bond: grief, hardship  ·  purged by yoga, ritual 4  ·  on 3 companions`

**Name**

<!--@ traits.json | haunted | name -->
Haunted
<!--@end-->


**Description**

<!--@ traits.json | haunted | description -->
Disturbing memories surface at the worst moments.
<!--@end-->


## lapsed

`skills: ritual-1  ·  pressure: space-10  ·  bond: solitary  ·  opposed: devout  ·  purged by ritual 3  ·  on 2 companions`

**Name**

<!--@ traits.json | lapsed | name -->
Lapsed
<!--@end-->


**Description**

<!--@ traits.json | lapsed | description -->
Once practiced, now adrift. The forms remain but the heart has gone.
<!--@end-->


## long_marched

`skills: logistics+1  ·  bond: order, hardship  ·  on 0 companions`

**Name**

<!--@ traits.json | long_marched | name -->
Long-marched
<!--@end-->


**Description**

<!--@ traits.json | long_marched | description -->
Has been on the road long enough to stop thinking of settling down.
<!--@end-->


## maimed

`pressure: earth-10  ·  bond: hardship  ·  on 0 companions`

**Name**

<!--@ traits.json | maimed | name -->
Maimed
<!--@end-->


**Description**

<!--@ traits.json | maimed | description -->
Something is gone that used to be there. Warmth still seems to linger in the void.
<!--@end-->


## mantra_worn

`skills: yoga+1, ritual+1  ·  pressure: space+10  ·  bond: devotion  ·  on 0 companions`

**Name**

<!--@ traits.json | mantra_worn | name -->
Mantra-worn
<!--@end-->


**Description**

<!--@ traits.json | mantra_worn | description -->
Has said it enough times that it says itself now, under everything else.
<!--@end-->


## merciful

`pressure: water+15  ·  bond: devotion  ·  opposed: blood_handed  ·  on 0 companions`

**Name**

<!--@ traits.json | merciful | name -->
Merciful
<!--@end-->


**Description**

<!--@ traits.json | merciful | description -->
Has spared someone who could not have stopped them.
<!--@end-->


## oath_breaker

`stats: charm-2  ·  pressure: earth-10  ·  bond: solitary  ·  opposed: oath_keeper  ·  purged by ritual 5  ·  on 0 companions`

**Name**

<!--@ traits.json | oath_breaker | name -->
Oath-breaker
<!--@end-->


**Description**

<!--@ traits.json | oath_breaker | description -->
Has broken a solemn vow. The weight of it is visible.
<!--@end-->


## oath_keeper

`skills: yoga+1  ·  pressure: earth+15  ·  bond: devotion, order  ·  opposed: oath_breaker  ·  on 0 companions`

**Name**

<!--@ traits.json | oath_keeper | name -->
Oath-keeper
<!--@end-->


**Description**

<!--@ traits.json | oath_keeper | description -->
Has held to a vow through the circumstances that made holding it expensive.
<!--@end-->


## renunciate

`skills: yoga+1  ·  pressure: fire+15, earth+5  ·  bond: devotion  ·  on 0 companions`

**Name**

<!--@ traits.json | renunciate | name -->
Renunciate
<!--@end-->


**Description**

<!--@ traits.json | renunciate | description -->
Has put down something they wanted badly, on purpose, and did not pick it back up.
<!--@end-->


## scarred

`bond: hardship  ·  on 0 companions`

**Name**

<!--@ traits.json | scarred | name -->
Scarred
<!--@end-->


**Description**

<!--@ traits.json | scarred | description -->
The wound closed. The body it closed on is not quite the one they had before.
<!--@end-->


## sole_survivor

`pressure: water-10, space-5  ·  bond: hardship, solitary  ·  purged by yoga, ritual 5  ·  on 0 companions`

**Name**

<!--@ traits.json | sole_survivor | name -->
Sole Survivor
<!--@end-->


**Description**

<!--@ traits.json | sole_survivor | description -->
Walked out of something that nobody else walked out of.
<!--@end-->


## steady_practice

`skills: yoga+1  ·  pressure: space+5  ·  bond: devotion, order  ·  on 0 companions`

**Name**

<!--@ traits.json | steady_practice | name -->
Steady Practice
<!--@end-->


**Description**

<!--@ traits.json | steady_practice | description -->
Sits every night, whether or not the night deserves it.
<!--@end-->


## touched_by_grace

`pressure: space+10, water+10  ·  bond: wonder, devotion  ·  on 0 companions`

**Name**

<!--@ traits.json | touched_by_grace | name -->
Touched by Grace
<!--@end-->


**Description**

<!--@ traits.json | touched_by_grace | description -->
Met something that had no business being kind to them, and was anyway.
<!--@end-->


## war_hardened

`stats: constitution+1  ·  pressure: water-10, air+5  ·  bond: martial, hardship  ·  purged by yoga 4  ·  on 3 companions`

**Name**

<!--@ traits.json | war_hardened | name -->
War-hardened
<!--@end-->


**Description**

<!--@ traits.json | war_hardened | description -->
Violence no longer shocks, only excites the senses.
<!--@end-->


---


# Racial  (28)


## ancient_memory

`stats: awareness+1  ·  skills: learning+1  ·  on 0 companions`

**Name**

<!--@ traits.json | ancient_memory | name -->
Ancient Memory
<!--@end-->


**Description**

<!--@ traits.json | ancient_memory | description -->
Long ages of accumulated experience have sharpened recall and insight. What others learn in a lifetime, this being has lived through many times over.
<!--@end-->


## aquatic

`pressure: water-5  ·  on 0 companions`

**Name**

<!--@ traits.json | aquatic | name -->
Aquatic
<!--@end-->


**Description**

<!--@ traits.json | aquatic | description -->
Born of the water. Breathes and moves freely in aquatic environments. If the entire party shares this trait, the party may traverse water tiles without penalty in both combat and overworld.
<!--@end-->


## armored

`stats: constitution+2, finesse-1  ·  on 0 companions`

**Name**

<!--@ traits.json | armored | name -->
Armored
<!--@end-->


**Description**

<!--@ traits.json | armored | description -->
Dense natural plating — chitin, thick hide, or bone — makes this creature hard to put down.
<!--@end-->


## better_starting_weapon

`on 0 companions`

**Name**

<!--@ traits.json | better_starting_weapon | name -->
Armory Rights
<!--@end-->


**Description**

<!--@ traits.json | better_starting_weapon | description -->
Some lives grant wisdom, others grief - this one grants you a superior weapon.
<!--@end-->


## bulky

`stats: constitution+1  ·  on 0 companions`

**Name**

<!--@ traits.json | bulky | name -->
Bulky
<!--@end-->


**Description**

<!--@ traits.json | bulky | description -->
A massive build that weathers punishment others could not survive.
<!--@end-->


## colony_mind

`pressure: fire-5, water-5, earth-5, air-5, space-5  ·  on 0 companions`

**Name**

<!--@ traits.json | colony_mind | name -->
Colony Mind
<!--@end-->


**Description**

<!--@ traits.json | colony_mind | description -->
Born into a collective, this creature is never truly alone. Shared purpose dampens individual anxiety.
<!--@end-->


## corpse_rider

`stats: focus+1  ·  skills: sorcery+1  ·  pressure: water-5  ·  on 0 companions`

**Name**

<!--@ traits.json | corpse_rider | name -->
Corpse Rider
<!--@end-->


**Description**

<!--@ traits.json | corpse_rider | description -->
Wears the dead the way others wear a coat, and has worn many. Something of each one stays behind in the way the hands move.
<!--@end-->


## deep_current

`stats: focus+1  ·  skills: water_magic+1  ·  pressure: water+5  ·  on 0 companions`

**Name**

<!--@ traits.json | deep_current | name -->
Deep Current
<!--@end-->


**Description**

<!--@ traits.json | deep_current | description -->
Thinks in pressures and distances no surface creature has words for. The magic that comes of it is correspondingly difficult to explain and difficult to argue with.
<!--@end-->


## extra_starting_gold

`on 0 companions`

**Name**

<!--@ traits.json | extra_starting_gold | name -->
Hoarder's Instinct
<!--@end-->


**Description**

<!--@ traits.json | extra_starting_gold | description -->
Maybe they inherited it, maybe they've hoarded it by themselves - the important part is the weight of the purse.
<!--@end-->


## first_to_know

`stats: awareness+1  ·  on 0 companions`

**Name**

<!--@ traits.json | first_to_know | name -->
First to Know
<!--@end-->


**Description**

<!--@ traits.json | first_to_know | description -->
The head comes up before the sound arrives. Alertness inherited rather than learned.
<!--@end-->


## flying

`stats: finesse+1  ·  pressure: air-5  ·  on 0 companions`

**Name**

<!--@ traits.json | flying | name -->
Flying
<!--@end-->


**Description**

<!--@ traits.json | flying | description -->
Sustained winged flight. If the entire party shares this trait, the party may traverse air tiles and elevated terrain in combat and overworld.
<!--@end-->


## guardian_geas

`stats: charm+1  ·  skills: ritual+1  ·  pressure: earth+5  ·  on 0 companions`

**Name**

<!--@ traits.json | guardian_geas | name -->
Guardian's Geas
<!--@end-->


**Description**

<!--@ traits.json | guardian_geas | description -->
Bound to a place, a hoard, or a promise made before memory. The binding is not resented; it is the shape of the self. Ask what is being guarded and the answer comes back older than the question.
<!--@end-->


## hell_born

`pressure: fire-5  ·  on 0 companions`

**Name**

<!--@ traits.json | hell_born | name -->
Hell-born
<!--@end-->


**Description**

<!--@ traits.json | hell_born | description -->
Forged in the fires and torments of hell. Instinctively at home in its rabid violence and hierarchy.
<!--@end-->


## hoard_keeper

`stats: awareness+1  ·  skills: trade+1  ·  pressure: earth+5  ·  on 0 companions`

**Name**

<!--@ traits.json | hoard_keeper | name -->
Hoard Keeper
<!--@end-->


**Description**

<!--@ traits.json | hoard_keeper | description -->
Wealth is not spent here, it is accumulated and remembered. Every piece of it has a provenance, and the provenance is the point.
<!--@end-->


## hunters_yoga

`stats: strength+1  ·  skills: yoga+1, unarmed+1  ·  pressure: fire-5  ·  on 0 companions`

**Name**

<!--@ traits.json | hunters_yoga | name -->
Hunter's Yoga
<!--@end-->


**Description**

<!--@ traits.json | hunters_yoga | description -->
Lifetimes of the twisted disciplines — breath held to stillness, hunger cultivated rather than sated, the body worked into a shape that only makes sense mid-leap. It is a real practice. It is simply pointed the wrong way.
<!--@end-->


## incorporeal

`stats: constitution-1  ·  pressure: earth-5  ·  on 0 companions`

**Name**

<!--@ traits.json | incorporeal | name -->
Incorporeal
<!--@end-->


**Description**

<!--@ traits.json | incorporeal | description -->
Barely present in the physical world — more dissolution than body. Food, wealth and violence seem somehow exotic.
<!--@end-->


## insatiable

`pressure: earth+10, water+5  ·  on 0 companions`

**Name**

<!--@ traits.json | insatiable | name -->
Insatiable
<!--@end-->


**Description**

<!--@ traits.json | insatiable | description -->
Mouth like a gate, throat like a needle.
<!--@end-->


## night_vision

`stats: awareness+1  ·  on 0 companions`

**Name**

<!--@ traits.json | night_vision | name -->
Night Vision
<!--@end-->


**Description**

<!--@ traits.json | night_vision | description -->
Sees equally in darkness and light, perceiving what others miss.
<!--@end-->


## nimble_mischief

`skills: guile+1  ·  on 0 companions`

**Name**

<!--@ traits.json | nimble_mischief | name -->
Nimble Mischief
<!--@end-->


**Description**

<!--@ traits.json | nimble_mischief | description -->
Quick hands and a quicker mind.
<!--@end-->


## predator_grace

`stats: finesse+2, constitution-1  ·  on 0 companions`

**Name**

<!--@ traits.json | predator_grace | name -->
Predator's Grace
<!--@end-->


**Description**

<!--@ traits.json | predator_grace | description -->
The natural speed and precision of a born hunter.
<!--@end-->


## serpentine

`pressure: water-5  ·  on 0 companions`

**Name**

<!--@ traits.json | serpentine | name -->
Serpentine
<!--@end-->


**Description**

<!--@ traits.json | serpentine | description -->
A body built for water and coiling grace, equally at home in the deep as on land.
<!--@end-->


## sovereign_grudge

`stats: charm+1  ·  skills: leadership+1  ·  pressure: space-5  ·  on 0 companions`

**Name**

<!--@ traits.json | sovereign_grudge | name -->
Sovereign Grudge
<!--@end-->


**Description**

<!--@ traits.json | sovereign_grudge | description -->
Ruled something once and has not accepted the loss of it. Every room is a court held in absentia, every slight a matter of record.
<!--@end-->


## swift_wing

`stats: finesse+1  ·  on 0 companions`

**Name**

<!--@ traits.json | swift_wing | name -->
Swift Wing
<!--@end-->


**Description**

<!--@ traits.json | swift_wing | description -->
Exceptional speed even among the creatures of the sky.
<!--@end-->


## taloned_strike

`skills: unarmed+1  ·  on 0 companions`

**Name**

<!--@ traits.json | taloned_strike | name -->
Taloned Strike
<!--@end-->


**Description**

<!--@ traits.json | taloned_strike | description -->
Natural weapons of uncommon sharpness.
<!--@end-->


## undead

`pressure: water-5  ·  on 0 companions`

**Name**

<!--@ traits.json | undead | name -->
Undead
<!--@end-->


**Description**

<!--@ traits.json | undead | description -->
The body is dead, or was never fully alive. Disease and poison find less purchase; the usual urgencies of flesh have gone quiet.
<!--@end-->


## venom_ward

`on 0 companions`

**Name**

<!--@ traits.json | venom_ward | name -->
Venom Ward
<!--@end-->


**Description**

<!--@ traits.json | venom_ward | description -->
Generations of hunting venomous prey have made this creature resistant to poisons.
<!--@end-->


## void_touched

`stats: focus+1  ·  skills: black_magic+1  ·  pressure: space-5  ·  on 0 companions`

**Name**

<!--@ traits.json | void_touched | name -->
Void Touched
<!--@end-->


**Description**

<!--@ traits.json | void_touched | description -->
Stood too near the place where things stop and came back with the cold still in them. Silence follows this one into rooms.
<!--@end-->


## war_banner

`stats: charm+1  ·  skills: leadership+1  ·  pressure: fire+5  ·  on 0 companions`

**Name**

<!--@ traits.json | war_banner | name -->
War Banner
<!--@end-->


**Description**

<!--@ traits.json | war_banner | description -->
Where this one stands, a line forms. It is not courage exactly — closer to the certainty that the fight has already been decided and everyone else is simply catching up.
<!--@end-->


---
