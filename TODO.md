...
- [ ] **595 perks still carry only prose** — `docs/review/PERK_FIXES.md`,
  generated by `tools/export_perk_fix_list.py`. Perks now take an `effects`
  block typed against `resources/data/perk_effects.json` (16 effect types, 22
  conditions); `stat`, `resistance`, `status_resistance`, `immunity` and `note`
  are implemented, 8 perks are migrated.

  The reason the other 595 are not is worth keeping in view: **of 603 perks only
  26 are unconditional standing bonuses.** Everything else fires while wielding
  something, against a kind of target, on a trigger, or on a school of spell.
  They cannot be migrated from their descriptions automatically because the
  condition is most of the content. So the work is not 595 migrations — it is
  implementing conditions in order of what each unlocks: `target_has_status` 56
  perks, `wielding` 47, `on_crit` 25, `adjacent_allies` 24. The document is
  organised that way.

  138 perks work today through hand-written `has_perk` checks in combat code.
  That is the pattern being replaced, and mixing the two applies a perk twice —
  `validate_data.py` now treats a perk that is both typed and hand-wired as an
  error.

  By shape: **105 actives** (each wants targeting, a cost, a UI affordance and an
  effect), **63 passive modifiers**, **50 out-of-combat** (shops, camp, travel,
  dialogue), 25 chance riders, and the rest bespoke. The actives are the ones
  that need more than a data block — an active perk is a button, and there is
