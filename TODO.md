...
- [ ] **Perk wiring (in progress)**: ~441 passive perks in perks.json; active skills (combat_data key) already work. Passive wiring is done in `get_passive_perk_stat_bonus()`, `_process_on_hit_perks()`, `_process_on_dodge_perks()`, `_process_turn_start_perks()`, `_process_spell_cast_perks()`, `_check_perk_status_immunity()`.
  ...
  *Simple / remaining (not yet wired):*
  ...
  - `tremor` (Earth 2) — Earth AoE spells 25% chance to knock prone. Hook in `_process_spell_cast_perks` checking `spell.get("targeting") == "aoe_*"`.
  - `crushing_depths` (Water 4) — enemies on water/ice terrain or with 2+ Water debuffs: -15% all saves. Hook in `_check_perk_status_immunity` (partial), `calculate_hit_chance` for accuracy bonus.
  - `avatar_of_the_storm` (Air 5) — all allies' attacks deal +5% Air bonus damage; 10% stun on hit. Hook in `_process_on_hit_perks` scanning allies for this perk.
  - `ashes_remember_heat` (Fire 5) — burning enemy killed → create burning terrain. Hook in `_process_on_hit_perks` + `_process_spell_cast_perks` on kill; call `combat_grid.set_terrain_effect(pos, "fire", 3)`.
  - `deep_freeze` (Water 3) — when Movement reduced to 0 by ice debuffs → apply Frozen. Check in `_process_turn_start_perks` for units with movement ≤ 0.
