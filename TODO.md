...
- [ ] **`special` is standardised in shape but not yet in content.**
  ...
  Batches A and B are done (2026-09-08): the 8 spells whose keys already carried
  their values are typed, and the 13 whose facts were filed in the wrong field
  are corrected — 18 descriptions of summoned creatures moved onto their entries
  in `summon_templates.json`, and the keys that restated the description or
  duplicated the `target`/`aoe` blocks are gone. **184 spells still carry a
  `legacy` block** (was 205) and **330 keys are frozen** (was 361).

  The remainder is design work, not renaming.
  ...
  **The per-spell worklist is `docs/review/SPELL_FIXES.md`**, generated by
  `tools/export_spell_fix_list.py`. Rerun it after any migration — a spell drops
  out of the document when its `legacy` block empties, so the counts are the
  progress bar. It sorts the 205 into four batches, easiest first:

  | Batch | Spells | What it needs |
  |---|---:|---|
  | ~~A — mechanical~~ | 0 | done |
  | ~~B — wrong field~~ | 0 | done |
  | C — needs a number | 94 | Real mechanic written as bare `true`. Pick the number. |
  | D — needs a decision | 90 | No registry type fits. |

  C is far fewer than 94 decisions because the keys repeat — the document leads
  with a table of every key used by more than one spell, so answering
  `freeze_chance` once migrates eight. D resolves largely into **four missing
  registry types**, which is the real design work: a **trigger wrapper** (nine
  keys want to say *when* an effect fires, and nothing can — probably a `when`
  param on every effect rather than a type), **damage typing** (damage carries
  no element at all today, which is a combat-system call), a **shield /
  damage-reduction** type with a source filter, and one **`resurrect`** type
  with an hp param that absorbs seven keys.
...
- [x] ~~**The stat pipeline.**~~ — built 2026-09-08, and it was a live bug rather
  than a missing feature. The `base_bonuses` tables hold percentages (Swords 10
  is `{"attack": 130, "damage": 130}`, i.e. +130%) and were being added as flat
  points, by both `character_system.gd` and a hand-copied twin in
  `enemy_system.gd`. A mid-game character had 117 damage from a 12-damage
  weapon and a hit chance pinned at the 95% cap.

  `resources/data/stat_keys.json` is the vocabulary that was missing: 44
  canonical stats, 32 legacy spellings mapped onto them, and a `mode` per stat
  saying how a modifier combines — `scaling` percentages multiply a computed
  base, `rate` percentages are the value itself, `flat` values are added.
  `update_derived_stats` gained a percentage stage; collection and application
  are public so the enemy path calls the same code. After the fix: damage 25 at
  Swords 5, 40 at Swords 10; hit chance 86% instead of 95%.

  Also fixed with it: `mana_cost` read as flat mana instead of a percentage;
  `weight_limit` from extra leg pairs added and then overwritten two lines
  later; Stone Adept's +10% max HP calling a `get_passive_perk_stat_bonus`
  branch nothing ever invoked.

- [ ] **595 perks still carry only prose.** Implementation instructions are in
  `docs/plans/PERK_WIRING_PLAN.md` — that file is written to be handed to a
  session cold. The per-perk inventory is `docs/review/PERK_FIXES.md`,
  regenerated by `tools/export_perk_fix_list.py`.

  **The actives were mis-prioritised and are the first batch, not the last.**
  `combat_arena.gd` already builds a skill button for any perk whose description
  begins with `Active`, and `CombatManager.use_active_skill` dispatches **44
  implemented effect types** with stamina, cooldowns, weapon requirements,
  targeting and AI usage scoring. **Not one perk carries a `combat_data` block**,
  so every one of those buttons reads "Skill not yet implemented" and the whole
  pipeline is unused. 109 perks of pure data authoring against working code.

  Everything else takes an `effects` block typed against `perk_effects.json` (17
  types, 22 conditions; 5 implemented, 8 perks migrated). Keep the two fields
  apart — `combat_data` works and folding it into the registry would break it.

  Batching is by **which system owns the hook**, because a batch is finishable
  exactly when it is one hook. After actives, the cheap one is `combat_stats`
  (75): the conditions already exist inline — `wielding` written out 28 times,
  `_unit_is_unarmored` at 6 call sites, adjacency at 27 — so the evaluator is
  mostly deletion. Its prerequisite is that `CombatUnit.get_accuracy()` and
  `get_spellpower()` have no perk hook at all, so no accuracy or spellpower perk
  can work today, hand-wired or not.

  Also fixed-in-passing candidate, recorded in the plan: the active-skill button
  reads its stamina cost from the description text while the executor charges
  `combat_data.stamina_cost`, and only 34 of the 109 actives state a cost in
  prose.

- [ ] **The affinity bonuses are still half-connected.** `get_affinity_bonuses`
  emits `damage_pct`, `healing_pct` and `mental_resistance_pct`, and
  `update_derived_stats` reads none of the three — it takes only the flat keys
  from that same dictionary. All three now exist in `stat_keys.json`, so this is
  a small change: route the affinity dictionary through
  `_accumulate_stat_modifier` the way the skill tables are, instead of reading
  named keys out of it.
