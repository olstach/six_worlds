...
## Build the battlefield from the sampled world tiles.
##
## The 260 lines this replaces counted the terrain around the party, turned
## the counts into obstacle budgets, and scattered them at random — so the
## field kept the proportions of the ground and lost its shape. The three
## passes live in BattlefieldGenerator, which is pure and therefore testable;
## this is only the handover.
func _generate_combat_terrain(context: Dictionary) -> Dictionary:
	var sample: Array = context.get("grid", [])
	if sample.is_empty():
		# No arrangement to work from (a scripted fight, a loaded save): fall
		# back to a field of the dominant ground rather than nothing at all.
		var g: int = int(context.get("dominant", 0))
		sample = []
		for _y in 5:
			sample.append([g, g, g, g, g])
	return BattlefieldGenerator.generate(sample, combat_grid.grid_size)
